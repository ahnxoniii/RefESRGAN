...
from utilsbsr import utils_blindsr as blindsr
import utilsbsr.utils_image as util
...
class RefCUFEDDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_in = mmcv.imfrombytes(img_bytes).astype(np.float32) / 255.  #numpy (0,1), BGR, float32
        ref_path = self.paths[index]['ref_path']
        img_bytes = self.file_client.get(ref_path, 'ref')
        img_ref = mmcv.imfrombytes(img_bytes).astype(np.float32)  #numpy (0,255), BGR, float32
        ...
        if self.opt['phase'] == 'train':
            ...
            #img_ref = img_ref * 255 #numpy 
            #img_ref = Image.fromarray( #PIL
            ...
            H_ref, W_ref, C_ref = img_ref.shape
            rnd_h_H_ref = random.randint(0, max(0, H_ref - gt_h))
            rnd_w_H_ref = random.randint(0, max(0, W_ref - gt_w))
            img_ref = img_ref[rnd_h_H_ref:rnd_h_H_ref + gt_h, rnd_w_H_ref:rnd_w_H_ref + gt_w, :] #numpy (0,255), BGR, float32
            
            #----------- random crop Ref -------------

            #img_ref = img_ref.resize((gt_w, gt_h), Image.BICUBIC)
            #img_ref = cv2.cvtColor(np.array(img_ref), cv2.COLOR_RGB2BGR)
            img_ref = img_ref / 255. #numpy (0,1), BGR, float32
            # data augmentation
            img_in, img_ref = augment([img_in, img_ref], self.opt['use_flip'],
                                      self.opt['use_rot']) #numpy (0,1), BGR, float32
            ...
            img_ref = mod_crop(img_ref, scale)#numpy (0,1), BGR, float32
            ...
            if img_in_h != img_ref_h or img_in_w != img_ref_w:
                ...
                img_ref = mmcv.impad(
                    img_ref, shape=(target_h, target_w), pad_val=0)#numpy (0,1), BGR, float32
        ...
        img_in_pil = img_in * 255 #numpy (0,255), BGR
        img_in_pil = Image.fromarray(
            cv2.cvtColor(img_in_pil.astype(np.uint8), cv2.COLOR_BGR2RGB)) #PIL, RGB, uint8

        img_ref_pil = img_ref * 255 #numpy (0,255), BGR
        img_ref_pil = Image.fromarray(
            cv2.cvtColor(img_ref_pil.astype(np.uint8), cv2.COLOR_BGR2RGB)) #PIL, RGB, uint8
        
        img_in_np = np.array(img_in_pil)
        img_in_np = img_in_np.astype(np.float32) / 255. #numpy (0,1), RGB, float32
        #print(f"img_in after shape: {img_in.shape}")
        
        #degradation
        if self.opt['phase'] == 'train':
            img_in_lq, img_in = blindsr.degradation_bsrgan(img_in_np, 4, lq_patchsize=72, isp_model=None, mode=0)  #numpy (0,1), RGB, float32
        else :
            img_in_lq, img_in = blindsr.degradation_bsrgan(img_in_np, 4, lq_patchsize=72, isp_model=None, mode=1)  #numpy (0,1), RGB, float32
        #print(f"img_in_lq min: {img_in_lq.min()}, max: {img_in_lq.max()}")
        #print(f"img_in min: {img_in.min()}, max: {img_in.max()}") 
        #print(f"img_in after shape: {img_in.shape}")
        #img_in = img_in * 255
        #img_in = img_in.astype(np.float32) / 255.
        #img_in = img_in.astype(np.float32) 
        #img_in_lq = img_in_pil.resize((lq_w, lq_h), Image.BICUBIC)
        img_ref_lq = img_ref_pil.resize((lq_w, lq_h), Image.BICUBIC) #PIL, RGB, uint8

        img_in_lq = img_in_lq * 255 #numpy (0,255), RGB, float32
        #img_in_lq = Image.fromarray(cv2.cvtColor(img_in_lq.astype(np.uint8), cv2.COLOR_BGR2RGB))
        img_in_lq = Image.fromarray(img_in_lq.astype(np.uint8)) #PIL, RGB, uint8

        # bicubic upsample LR
        img_in_up = img_in_lq.resize((gt_w, gt_h), Image.BICUBIC) #PIL, RGB
        img_ref_up = img_ref_lq.resize((gt_w, gt_h), Image.BICUBIC) #PIL, RGB
        
        img_in = img_in * 255 #numpy (0,255), RGB, float32
        img_in = Image.fromarray(img_in.astype(np.uint8)) #PIL, RGB, uint8

        #list input : PIL, RGB, unit8 output : numpy (0,1), BGR
        img_in_lq = cv2.cvtColor(np.array(img_in_lq), cv2.COLOR_RGB2BGR)
        img_in_lq = img_in_lq.astype(np.float32) / 255. #numpy (0,1), BGR
        img_in_up = cv2.cvtColor(np.array(img_in_up), cv2.COLOR_RGB2BGR)
        img_in_up = img_in_up.astype(np.float32) / 255. #numpy (0,1), BGR
        img_ref_lq = cv2.cvtColor(np.array(img_ref_lq), cv2.COLOR_RGB2BGR)
        img_ref_lq = img_ref_lq.astype(np.float32) / 255. #numpy (0,1), BGR
        img_ref_up = cv2.cvtColor(np.array(img_ref_up), cv2.COLOR_RGB2BGR)
        img_ref_up = img_ref_up.astype(np.float32) / 255. #numpy (0,1), BGR
        img_in = cv2.cvtColor(np.array(img_in), cv2.COLOR_RGB2BGR)
        img_in = img_in.astype(np.float32) / 255. #numpy (0,1), BGR
        ...
        return_dict = {
            'img_in': img_in, #HR after augument 
            'img_in_lq': img_in_lq, #LR
            'img_in_up': img_in_up, #LR+up
            'img_ref': img_ref, #Ref
            'img_ref_lq': img_ref_lq, #Ref+lq
            'img_ref_up': img_ref_up, #Ref+up
        }
