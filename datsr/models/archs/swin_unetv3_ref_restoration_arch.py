...
class ContentExtractor(nn.Module):
    '''
    conv - relu - conv .... feature extractor
    '''
    ...
    def forward(self, x):
        feat = self.lrelu(self.conv_first(x)) # 3 -> 64
...
class ResidualDenseBlock_5C(nn.Module):
    def __init__(self, nf=64, gc=32, bias=True): #nf: 입력 채널 gc: 출력 채널 
...
class SwinUnetv3RestorationNet(nn.Module):
    def __init__(self, ngf=128, n_blocks=16, groups=8, embed_dim=64, depths=(8,8), num_heads=(8,8), window_size=8, use_checkpoint=False):
    ...
    def forward(self, x, pre_offset_flow_sim, img_ref_feat):
        ...
        base = x # [B, 3, 40, 40]
        #base = F.interpolate(x, None, 4, 'bilinear', False) # [B, 3, 160, 160]
        #content_feat = self.content_extractor(x) # [B, 64, 40, 40]
...
class DynamicAggregationRestoration(nn.Module):
    ...
    def __init__(self,
        ...
        self.patch_norm = patch_norm
        self.mlp_ratio = mlp_ratio
        self.unet_head = nn.Conv2d(3, ngf, kernel_size=3, stride=1, padding=1) #[b, 64, s, s]

        # -------------------------- ESRGAN --------------------------
        # -------------------------- ESRGAN --------------------------

        RRDB_block_f = functools.partial(RRDB, nf=64, gc=32) #rrdb block
        self.sf = 4
        self.RRDB_trunk = make_layer(RRDB_block_f, 20) #nb만큼 rrdb 블럭 레이어를 만듬
        self.trunk_conv = nn.Conv2d(ngf, ngf, 3, 1, 1, bias=True)
        #### upsampling
        self.upconv1 = nn.Conv2d(ngf, ngf, 3, 1, 1, bias=True)
        if self.sf==4:
            self.upconv2 = nn.Conv2d(ngf, ngf, 3, 1, 1, bias=True)
        self.HRconv = nn.Conv2d(ngf, ngf, 3, 1, 1, bias=True)
        self.conv_last = nn.Conv2d(ngf, 3, 3, 1, 1, bias=True)
        self.conv1X1 = nn.Conv2d(256, 64, 1, 1, 0)
    ...
    def forward(self, base, pre_offset_flow_sim, img_ref_feat):
        ...
        pre_relu3_swapped_feat = self.flow_warp(img_ref_feat['relu3_1'], pre_flow['relu3_1']) # [B, 256, 40, 40] 
        # base : [B, 3, 40, 40]
        x0 = self.unet_head(base)    # [B, 64, 40, 40] batch channel h w

        # --------------- RDA -----------------
        # pre_relu3_swapped_feat = self.conv1X1(pre_relu3_swapped_feat) # [B, 64, 40, 40]
        # img_ref_feat_relu_3_1 = self.conv1X1(img_ref_feat['relu3_1']) # [B, 64, 40, 40]
        # pre_relu3_swapped_feat : [B, 256, 40, 40]
        # img_ref_feat['relu3_1']: [B, 256, 40, 40]
        # print(f"x0 shape: {x0.size()}")
        # print(f"pre_relu3_swapped_feat shape: {pre_relu3_swapped_feat.size()}")
        # print(f"img_ref_feat['relu3_1'] shape: {img_ref_feat['relu3_1'].size()}")

        relu3_offset = torch.cat([x0, pre_relu3_swapped_feat, img_ref_feat['relu3_1']], 1) # [B, 64+ 256 *2, 40, 40]
        ...
        relu3_swapped_feat = self.lrelu(
            self.up_small_dyn_agg([img_ref_feat['relu3_1'], relu3_offset], pre_offset['relu3_1'], pre_similarity['relu3_1'])) #A_l # [B, 256, 40, 40]

        # ------------- RFA -> ESRGAN --------------
        h = torch.cat([x0, relu3_swapped_feat], 1) #RFA start # [B, 64 + 256, 40, 40]

        h = self.down_head_large(h) # [B, 64, 40, 40]
        trunk = self.trunk_conv(self.RRDB_trunk(h)) #dense block # [B, 64, 40, 40]
        #fea = h + trunk # concat + residual  # [B, 64, 40, 40]
        fea = x0 + trunk # blue LR + residual
        fea = self.lrelu(self.upconv1(F.interpolate(fea, scale_factor=2, mode='nearest')))  #nearest로 업샘플링 후 컨벌루션 # [B, 64, 80, 80]
        if self.sf == 4:
             fea = self.lrelu(self.upconv2(F.interpolate(fea, scale_factor=2, mode='nearest'))) #4배 # [B, 64, 160, 160]
        x = self.conv_last(self.lrelu(self.HRconv(fea))) #최종 컨벌루션 # [B, 64, 160, 160] -> [B, 3, 160, 160]
