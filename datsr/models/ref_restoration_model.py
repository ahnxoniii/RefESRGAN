...
class RefRestorationModel(SRModel):
    ...
    def feed_data(self, data):
        ...
        # print(f"img_lq is of type {type(self.img_in_lq)}")
        # print(f"img_lq min: {self.img_in_lq.min()}, max: {self.img_in_lq.max()}") 
        # print(f"img_lq shape: {self.img_in_lq.shape}")
        # print(f"img_gt is of type {type(self.gt)}")
        # print(f"img_gt min: {self.gt.min()}, max: {self.gt.max()}") 
        # print(f"img_gt shape: {self.gt.shape}")
        # print(f"img_ref is of type {type(self.img_ref)}")
        # print(f"img_ref min: {self.img_ref.min()}, max: {self.img_ref.max()}") 
    ...
    def optimize_parameters(self, step):
        ...
        if step <= self.net_g_pretrain_steps:
            ...
            if self.net_d:
                ...
                # compute WGAN loss
                pred_d_fake = self.net_d(self.output).detach()   
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            # Print individual PSNR for each image
            logger.info(f'# Image {img_name} # PSNR: {psnr:.4e}')
        ...
        # log individual PSNR to TensorBoard
        if tb_logger:
            for i, psnr in enumerate(psnr_list):
                tb_logger.add_scalar(f'psnr_image_{i}', psnr, current_iter)
        ...
        with open("/home/ivc2411/DATSRBSRGAN/experiments/test/results/results.txt", "a") as f:  # 'a' 모드는 기존 파일에 추가
            f.write(f'# {num} PSNR: {avg_psnr:.4e}, # PSNR_Y: {avg_psnr_y:.4e} # SSIM_Y: {avg_ssim_y:.4e}.\n')
